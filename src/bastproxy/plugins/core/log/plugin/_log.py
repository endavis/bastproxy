...
class LogPlugin(BasePlugin):
    ...
    def _phook_initialize(self):
        ...
        self.api("plugins.core.settings:add")(
            self.plugin_id, "color_info", "@w", "color", "the color for info messages"
        )
        self.api("plugins.core.settings:add")(
            self.plugin_id,
            "color_debug",
    ...
    def _api_set_log_to_client(self, logtype, level: str = "info", flag=True):
        ...
        """
        LogRecord(
            f"_api_set_log_to_client: {logtype} {level} {flag}",
            level="debug",
        ...
        if not level:
            LogRecord(
                f"api_toggletoclient: invalid log level {level}",
                level="error",
                sources=[self.plugin_id, logger_name],
            )()
            return
        ...
        if flag and logger_name not in self.handlers["client"]:
            ...
            LogRecord(
                f"removing {logger_name} logging to client {level}",
                level="debug",
                sources=[self.plugin_id, logger_name],
            )()
    ...
    def _command_client(self):
        ...
        if args["logtype"]:
            ...
            for i in args["logtype"]:
                ...
                self.api(f"{self.plugin_id}:set.log.to.client")(
    ...
    def _api_set_log_to_console(self, logtype, level="info"):
        ...
        """
        LogRecord(
            f"_api_set_log_to_console: {logtype} {level}",
            level="debug",
            sources=[self.plugin_id],
        )()
        ...
        if not level:
            LogRecord(
                f"_api_set_log_to_console: invalid log level {level}",
                level="error",
                sources=[self.plugin_id, logger_name],
            )()
            return
        ...
        LogRecord(
            f"setting {logger_name} to log to console at level {level}",
            level="debug",
            sources=[self.plugin_id, logger_name],
        )()
    ...
    def _api_set_log_to_file(self, logtype, level="info"):
        ...
        if not level:
            LogRecord(
                f"_api_set_log_to_file: invalid log level {level}",
                level="error",
                sources=[self.plugin_id, logtype],
            )()
            return
        ...
        LogRecord(
            f"setting {logger_name} to log to file at level {level}",
            level="debug",
            sources=[self.plugin_id, logger_name],
        )()
    ...
    @AddArgument("logtype", help="the logtype to toggle", default="list", nargs="?")
    @AddArgument(
        "-n",
        "--notimestamp",
        ...
        if types:
            ...
            for i in types:
                if i in self.type_counts:
                    tmsg.append(
                        f"{i:<{self.logtype_col_length}} : {self.type_counts[i]['debug']:<5} {self.type_counts[i]['info']:<5} {self.type_counts[i]['warning']:<7} {self.type_counts[i]['error']:<5} {self.type_counts[i]['critical']:<8}"
                    )
                else:
                    tmsg.append(
                        f"{i:<{self.logtype_col_length}} : {'0':<5} {'0':<5} {'0':<7} {'0':<5} {'0':<8}"
                    )
        else:
    ...
    @AddArgument("message", help="the text to log")
    @AddArgument("-l", "--logtype", help="the facility to test", required=True)
    @AddArgument(
        "-ll",
        "--loglevel",
    ...
    def _api_clean_types(self):
        ...
        for i in types:
            if i not in self.type_counts or max(self.type_counts[i].values()) <= 0:
                remove.append(i)
