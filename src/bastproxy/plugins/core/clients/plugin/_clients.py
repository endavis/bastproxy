...
class BanRecord:
    def __init__(
        self, plugin_id: str, ip_addr: str, how_long: int = 600, copy: bool = False
    ):
    ...
    def expires(self) -> str:
        if next_fire := self.api("plugins.core.timers:get.timer.next.fire")(
            self.timer_name
        ):
...
class ClientPlugin(BasePlugin):
    ...
    def _phook_initialize(self):
        ...
        self.api("plugins.core.events:add.event")(
            f"ev_{self.plugin_id}_client_logged_in",
            self.plugin_id,
            description=["An event that is raised when a client logs in"],
            arg_descriptions={"client_uuid": "the uuid of the client"},
        )
        self.api("plugins.core.events:add.event")(
            f"ev_{self.plugin_id}_client_logged_in_view_only",
            self.plugin_id,
            description=[
                "An event that is raised when a client logs in as a view client"
            ],
            arg_descriptions={"client_uuid": "the uuid of the client"},
        )
        self.api("plugins.core.events:add.event")(
            f"ev_{self.plugin_id}_client_connected",
            self.plugin_id,
            description=["An event that is raised when a client connects"],
            arg_descriptions={"client_uuid": "the uuid of the client"},
        )
        self.api("plugins.core.events:add.event")(
            f"ev_{self.plugin_id}_client_disconnected",
            self.plugin_id,
        ...
        if self.banned:
            LogRecord(
                f"Loading {len(self.banned)} banned IPs",
                level="debug",
        ...
        if how_long == -1:
            ...
            if ip_address not in permbanips:
                ...
                self.api("plugins.core.settings:change")(
                    self.plugin_id, "permbanips", permbanips
                )
                LogRecord(
                    f"{ip_address} has been banned with no expiration",
                    level="error",
                    sources=[self.plugin_id],
                )()
                return True
            ...
            LogRecord(
                f"{ip_address} has been automatically banned for {how_long} seconds",
                level="error",
                sources=[self.plugin_id],
            )()
            return True
    ...
    def _api_client_logged_in(self, client_uuid):
        ...
        if client_uuid in self.clients:
            ...
            LogRecord(
                f"Client {client_connection.uuid} logged in from {client_connection.addr}:{client_connection.port}",
                level="warning",
                sources=[self.plugin_id],
            )()
    ...
    def _api_client_logged_in_view_only(self, client_uuid):
        ...
        if client_uuid in self.clients:
            ...
            LogRecord(
                f"View Client {client_connection.uuid} logged in from {client_connection.addr}:{client_connection.port}",
                level="warning",
                sources=[self.plugin_id],
            )()
    ...
    def _api_client_remove(self, client_connection):
        ...
        if client_connection.uuid in self.clients:
            ...
            LogRecord(
                f"Client {client_connection.uuid} disconnected {client_connection.addr}:{client_connection.port}",
                level="warning",
                sources=[self.plugin_id],
            )()
    ...
    def _command_show(self):
        ...
        clients = [
            {
                ...
                "port": client.port,
                "term_type": "Term Type",
                "connected": client.connected_length,
                "view_only": str(client.view_only),
            }
        ...
        tmsg.extend(
            self.api("plugins.core.utils:convert.data.to.output.table")(
                "Clients", clients, clients_columns, color=color
            )
        )
        ...
        if banned_clients:
            ...
            tmsg.extend(
                self.api("plugins.core.utils:convert.data.to.output.table")(
                    "Banned IPs", banned_clients, banned_clients_columns, color=color
                )
            )
    ...
    def _command_ban(self):
        ...
        if removed:
            tmsg.extend(
                (f"Removed {len(removed)} IPs from the ban list", ", ".join(removed))
            )
        if added:
            tmsg.extend((f"Added {len(added)} IPs to the ban list", ", ".join(added)))
        if not_found:
            tmsg.extend(
                (f"{len(not_found)} IPs were not acted on", ", ".join(not_found))
            )
