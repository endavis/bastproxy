...
class EventsPlugin(BasePlugin):
    ...
    def _register_all_plugin_events(self):
        ...
        for plugin_id in self.api("libs.plugins.loader:get.loaded.plugins.list")():
            LogRecord(
                f"_register_all_plugin_events: registering events in {plugin_id}",
                level="debug",
                sources=[self.plugin_id],
            )()
            self._register_events_for_plugin(plugin_id)
        LogRecord(
            "_register_all_plugin_events: finish",
            level="debug",
            sources=[self.plugin_id],
        )()
    ...
    def _register_events_for_plugin(self, plugin_id):
        ...
        if event_functions:
            ...
            LogRecord(
                f"_register_events_for_plugin {names = }",
                level="debug",
                sources=[self.plugin_id, plugin_id],
            )()
    ...
    def _api_register_to_event(self, event_name, func, **kwargs):
        ...
        if not func_owner_id:
            LogRecord(
                f"_api_register_to_event - could not find owner for {func}",
                level="error",
                sources=[self.plugin_id],
            )()
            return
    ...
    def _api_unregister_from_event(self, event_name, func):
        ...
        if event_name in self.events:
            ...
            LogRecord(
                f"_api_unregister_from_event - could not find event {event_name}",
                level="error",
                sources=[self.plugin_id],
            )()

    @AddAPI(
        "remove.events.for.owner",
    ...
    def _api_raise_event(
        ...
        if not calledfrom:
            LogRecord(
                f"event {event_name} raised with unknown caller",
                level="warning",
                sources=[self.plugin_id],
            )()
        ...
        raised_event = event.raise_event(
            event_args, calledfrom, data_list=data_list, key_name=key_name
        )
