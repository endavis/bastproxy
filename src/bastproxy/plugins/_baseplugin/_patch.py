...
def patch(full_import_location, override=False):
    ...
    if full_import_location not in sys.modules:
        try:
            ...
            LogRecord(
                f"Could not load module {full_import_location} for patching of base",
                level="warning",
                sources=["baseplugin"],
                exc_info=True,
            )()
    ...
    if module.__name__ in modules_patched and not override:
        LogRecord(
            f"module {module.__name__} has already patched base",
            level="warning",
            sources=["baseplugin"],
        )()
        return False
    ...
    LogRecord(
        f"patching base from module {module.__name__}",
        level="info",
        sources=["baseplugin"],
    )()
    for item in dir(module):
        ...
        if isinstance(itemo, types.FunctionType) and item.startswith("_"):
            if hasattr(Plugin, itemo.__name__):
                LogRecord(
                    f"skipping {module.__name__}:{itemo.__name__} as it already exists in base",
                    level="warning",
                    sources=["baseplugin"],
                )()
                continue
