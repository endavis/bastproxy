...
class TelnetServer(server_base.BaseServer):
    ...
    def connection_made(self, transport):
        logger.debug("TelnetServer.connection_made")
        ...
        for tel_opt, callback_fn in [
            ...
            logger.debug("TelnetServer.connection_made: tel_opt=%s", tel_opt)
        ...
        logger.debug("TelnetServer.connection_made done")
...
async def create_server(host=None, port=23, protocol_factory=TelnetServer, **kwds):
    ...
    :return asyncio.Server: The return value is the same as
        ...
        to stop the service.
    """
    logger.debug("create_server: host={0}, port={1}".format(host, port))
