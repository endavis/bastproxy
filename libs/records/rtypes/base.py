...
class BaseRecord(AttributeMonitor):
    ...
    def _am_onchange__all(self, name, original_value, new_value):
        ...
        self.addupdate(
            ...
            extra={"original": original_value, "new": new_value},
        )
    ...
    def get_all_updates(self, update_filter=None) -> list[UpdateRecord]:
        ...
        for record in RMANAGER.get_all_children_list(self, record_filter=update_filter):
            updates.extend(
                ...
                if update.parent.__class__.__name__ not in update_filter
            )
    ...
    def get_attributes_to_format(self):
        ...
        for item in self._attributes_to_monitor:
            ...
            if orig_value == getattr(self, item):
                ...
                default_attributes[0].append(
                    (
                        item,
                        f"changed from '{self._am_get_original_value(item)!r}' to '{getattr(self, item)!r}'",
                    )
                )
    ...
    def format_children(self, full_children_records=False, update_filter=None):
        ...
        if full_children_records:
            ...
            for record in children_records:
                msg.extend(
                    f"     {line}"
                    for line in record.get_formatted_details(
                        full_children_records=False,
        ...
        for level in attributes:
            for item_string, item_attr in attributes[level]:
                ...
                if isinstance(attr, (list, dict)):
                    ...
                    msg.extend(
                        f"{'':<15} : {line}"
                        for line in pprint.pformat(attr, width=120).splitlines()
                    )
                else:
    ...
    def __call__(self, *args, **kwargs):
        ...
        if self.executing:
            self.addupdate(
                "Info",
                f"{self.__class__.__name__} already executing",
                extra={"actor": actor},
            )
            return
        ...
        if self.track_record:
            tracking_uuid = self.api("libs.timing:start")(
                f"{self.__class__.__name__}:{self.uuid}.__call__"
            )
            RMANAGER.start(self)
...
class TrackedUserList(BaseRecord, UserList):
    ...
    def __setitem__(self, index, item):
        ...
        if self.locked:
            self.addupdate(
                "Info",
                f"Attempted to update a locked data list at index {index} with value '{item!r}'",
            )
            return
    ...
    def insert(self, index, item):
        ...
        if self.locked:
            self.addupdate(
                "Info",
                f"Attempted to insert into a locked data list at index {index} with value '{item!r}'",
            )
            return
        super().insert(index, item)
        self.addupdate(
            "Modify",
            f"inserted item into position {index}",
            extra={"item": f"{item!r}"},
        )
    ...
    def append(self, item):
        ...
        if self.locked:
            self.addupdate(
                "Info",
                f"Attempted to append to a locked data list at index with value '{item!r}'",
            )
            return
        super().append(item)
        self.addupdate(
            "Modify",
            f"Appended item into position {len(self) - 1}",
            extra={"item": f"{item!r}"},
        )
    ...
    def extend(self, items: list):
        ...
        if self.locked:
            self.addupdate(
                "Info",
                f"Attempted to extend a locked data list '{[repr(item) for item in items]}'",
            )
            return
        super().extend(items)
        self.addupdate(
            "Modify",
            "extended list",
            extra={"added_list": f"{[repr(item) for item in items]}"},
        )
...
class BaseListRecord(UserList, BaseRecord):
    ...
    def add_line_endings(self, actor=""):
        ...
        self.replace(
            new_message,
            f"{actor}:add_line_endings",
            extra={"msg": "add line endings to each item"},
        )
    ...
    def color_lines(self, color: str, actor=""):
        ...
        self.replace(
            new_message,
            actor=f"{actor}:color_lines",
            extra={"msg": "convert color codes to ansi codes on each item"},
        )
