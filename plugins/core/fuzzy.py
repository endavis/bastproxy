...
class Plugin(BasePlugin):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.api('libs.api:add')('get:top:matches', self.get_top_matches)
    ...
    def get_best_match(self, item_to_match, list_to_match, score_cutoff = 80,
        ...
        LogRecord(f"get_best_match - {item_to_match=}, {scorer=} {score_cutoff=}",
                  level='debug', sources=[self.plugin_id]).send()
        LogRecord(f"get_best_match - {list_to_match=}",
                  level='debug', sources=[self.plugin_id]).send()
        if item_to_match in list_to_match:
            found = item_to_match
            LogRecord(f"get_best_match (exact) matched {item_to_match} to {found}",
                      level='debug', sources=[self.plugin_id]).send()
        else:
    ...
    def get_top_matches(self, item_to_match, list_to_match, items=5, score_cutoff=80, scorer: str = 'ratio') -> list:
        """
        scorer can be ratio, partial_ratio, token_sort_ratio,
                        token_set_ratio, partial_token_sort_ratio,
                        partial_token_set_ratio
