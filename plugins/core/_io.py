...
NAME = 'io'
SNAME = 'io'
PURPOSE = 'handle different types of IO'
AUTHOR = 'Bast'
VERSION = 1
...
class Plugin(BasePlugin):
  """
  plugin for IO in the proxy
    APIs for this class
     'core.io:send:msg'       : send data through the messaging system for
                                logging purposes
     'core.io:send:error'     : send an error
     'core.io:send:traceback' : send a traceback
     'core.io:send:client'    : send data to the clients
     'core.io:send:mud'       : send data to the mud
     'core.io:send:execute'   : send data through the parser
  """
  def __init__(self, *args, **kwargs):
    ...
    self.api('libs.api:add')('send', 'msg', self._api_msg)
    self.api('libs.api:add')('send', 'error', self._api_error)
    self.api('libs.api:add')('send', 'traceback', self._api_traceback)
    self.api('libs.api:add')('send', 'client', self._api_client)
    self.api('libs.api:add')('send', 'mud', self._api_tomud)
    self.api('libs.api:add')('send', 'execute', self._api_execute)
