...
class CommandsPlugin(BasePlugin):
    ...
    def _phook_init_plugin(self):
        ...
        self.default_help_command = {
            "plugin_id": "plugins.core.pluginm",
            "command": "list",
        }

        # load the history
        self.history_save_file = self.plugin_info.data_directory / "history.txt"
        self.command_history_dict = PersistentDict(
            self.plugin_id, self.history_save_file, "c"
        )
    ...
    def _phook_initialize(self):
        ...
        self.api("plugins.core.settings:add")(
            self.plugin_id, "historysize", 50, int, "the size of the history to keep"
        )
        self.api("plugins.core.settings:add")(
            self.plugin_id,
            "header_color",
        ...
        self.api("plugins.core.settings:add")(
            self.plugin_id,
            "output_subheader_color",
            "@G",
            "color",
            "the color to use for a subheader in the output of a command",
        )
        self.api("plugins.core.settings:add")(
            self.plugin_id, "command_indent", 0, int, "the indent for a command"
        )
        self.api("plugins.core.settings:add")(
            self.plugin_id,
            "simple_output",
        ...
        self.api("plugins.core.settings:add")(
            self.plugin_id, "multiline_headers", True, bool, "headers are multiline"
        )
        self.api("plugins.core.settings:add")(
            self.plugin_id,
            "include_date",
    ...
    def _api_get_output_indent(self):
        ...
        return (
            self.api("plugins.core.settings:get")(self.plugin_id, "command_indent") * 2
        )
    ...
    def _add_commands_for_all_plugins(self):
        ...
        for plugin_id in self.api("libs.plugins.loader:get.loaded.plugins.list")():
            LogRecord(
                f"_eventdb_add_commands_on_startup: loading commands for {plugin_id}",
                level="debug",
                sources=[self.plugin_id],
            )()
            self.update_commands_for_plugin(plugin_id)
        LogRecord(
            "_eventcb_add_commands_on_reload: Finish",
            level="debug",
            sources=[self.plugin_id],
        )()
    ...
    def _eventcb_plugin_loaded(self):
        ...
        if not self.api.startup and event_record["plugin_id"] == self.plugin_id:
            ...
            LogRecord(
                f"_eventcb_plugin_loaded: loading commands for {event_record['plugin_id']}",
                level="debug",
                sources=[self.plugin_id],
            )()
    ...
    def update_commands_for_plugin(self, plugin_id):
        ...
        LogRecord(
            f"update_commands_for_plugin: {plugin_id} has {len(command_functions)} commands",
            level="debug",
            sources=[self.plugin_id],
        )()
        if command_functions:
            command_names = [command.__name__ for command in command_functions]
            LogRecord(f"{command_names = }", level="debug", sources=[self.plugin_id])()
    ...
    def _api_add_command_by_func(self, func, force=False):
        ...
        if not (
            ...
            and func.__name__.startswith("_command_")
            and hasattr(func, "command_data")
        ):
            LogRecord(
                f"Function is not a command: {func.__name__}",
                level="warning",
                sources=[self.plugin_id],
            )()
            return
        ...
        if "description" in command_data.argparse["kwargs"]:
            command_data.argparse["kwargs"]["description"] = command_data.argparse[
                "kwargs"
            ]["description"].format(**func.__self__.__dict__)

        parser = argp.ArgumentParser(**command_data.argparse["kwargs"])
        for arg in command_data.arguments:
            arg["kwargs"]["help"] = arg["kwargs"]["help"].format(
        ...
        parser.prog = self.api("plugins.core.commands:get.command.format")(
        ...
        try:
            command = CommandClass(
                plugin_id, command_name, func, parser, **command_kwargs
            )
        except Exception:
            LogRecord(
                f"Error creating command {func.__name__}: {command_kwargs}",
                level="error",
                sources=[self.plugin_id],
                exc_info=True,
            )()
        ...
        LogRecord(
            f"added command {plugin_id}.{command_name}",
            level="debug",
    ...
    def _eventcb_plugin_unloaded(self):
        ...
        if event_record := self.api("plugins.core.events:get.current.event.record")():
            if event_record["plugin_id"] != self.plugin_id:
                ...
                LogRecord(
                    f"{self.plugin_id}_plugin_unloaded: {self.plugin_id} is me!",
                    level="debug",
                    sources=[self.plugin_id],
                )()
    ...
    @AddAPI(
        "remove.data.for.plugin", description="remove all command data for a plugin"
    )
    def _api_remove_data_for_plugin(self, plugin_id):
        ...
        """
        LogRecord(
            f"removing commands for plugin {plugin_id}",
            level="debug",
            sources=[self.plugin_id, plugin_id],
        )()
        if self.api("libs.plugins.loader:is.plugin.id")(plugin_id):
            ...
            new_commands = [
                ...
                if not command.startswith(plugin_id)
            ]
    ...
    @AddAPI(
        "get.commands.for.plugin.data", description="get the command data for a plugin"
    )
    ...
    def _api_run(
        ...
        """
        LogRecord(
            f"running command {command_name} from plugin {plugin_id} with arguments {argument_string}",
            level="debug",
            sources=[self.plugin_id, plugin_id],
        ...
        if command := self.get_command_data_from_plugin(plugin_id, command_name):
            ...
            if not success:
                LogRecord(
                    f"command {command_name} from plugin {plugin_id} failed with message {message}",
                    level="debug",
                    sources=[self.plugin_id, plugin_id],
                    stack_info=True,
                )()
        ...
        LogRecord(
            f"command {command_name} from plugin {plugin_id} not found",
            level="warning",
            sources=[self.plugin_id, plugin_id],
            stack_info=True,
        )()
    ...
    def add_command_to_history(self, command: str):
        ...
        if len(self.command_history_data) >= self.api("plugins.core.settings:get")(
            self.plugin_id, "historysize"
        ):
    ...
    def update_command(self, plugin_id, command_name, command: CommandClass):
        ...
        if not self.api("libs.plugins.loader:is.plugin.id")(plugin_id):
            LogRecord(
                f"commands - update_command: plugin {plugin_id} does not exist",
                level="debug",
                sources=[plugin_id, self.plugin_id],
            )(f"{self.plugin_id}:update_command")
            return False
        ...
        if command_name not in plugin_command_data and not self.api.startup:
            LogRecord(
                f"commands - update_command: plugin {plugin_id} does not have command {command_name}",
                level="debug",
                sources=[plugin_id, self.plugin_id],
            )()
    ...
    def pass_through_command_from_event(self) -> None:
        ...
        if original_command == self.api("plugins.core.settings:get")(
            self.plugin_id, "lastcmd"
        ):
            ...
            # command in between
            if self.api("plugins.core.settings:get")(
                self.plugin_id, "cmdcount"
            ) == self.api("plugins.core.settings:get")(self.plugin_id, "spamcount"):
                event_record.addupdate(
                    "Modify", "Antispam Command sent", savedata=False
                )
                LogRecord(
                    f"sending antspam command: {self.api('plugins.core.settings:get')('plugins.core.commands', 'antispamcommand')}",
                    level="debug",
                    sources=[self.plugin_id],
                )()
                SendDataDirectlyToMud(
                    NetworkData(
                        self.api("plugins.core.settings:get")(
                            self.plugin_id, "antispamcommand"
                        )
                    ),
            ...
            if original_command in self.no_multiple_commands:
                event_record.addupdate(
                    "Modify",
                    "this command has been flagged to only be sent once, sendtomud set to False",
                    savedata=False,
                )
            ...
            self.api("plugins.core.settings:change")(self.plugin_id, "cmdcount", 0)
            LogRecord(
                f"resetting command to {original_command}",
                level="debug",
                sources=[self.plugin_id],
            )()
            self.api("plugins.core.settings:change")(
                self.plugin_id, "lastcmd", original_command
            )
    ...
    def error_command_not_found_in_plugin(self, plugin_id, command=None):
        ...
        message.extend(
            [
                *[
                    line.replace("plugins.", "")
                    for line in self.api(f"{self.plugin_id}:list.commands.formatted")(
                        plugin_id
                    )
                ],
            ]
    ...
    def error_plugin_not_found(self, plugin_id=None, package=None):
        ...
        plugins = [
            line.replace("plugins.", "")
            for line in self.api("libs.plugins.loader:get.plugins.in.package")(package)
        ]

        message.extend(
            [
                *self.api(f"{self.plugin_id}:format.output.header")(
                    f"Available Plugins in {package}"
                ),
    ...
    def error_package_not_found(self, package=None):
        ...
        message = [
            ...
            *[
                line.replace("plugins.", "")
                for line in self.api("libs.plugins.loader:get.packages.list")(
                    active_only=True
                )
            ],
        ]
    ...
    def find_command(
        ...
        )(actor=f"{self.plugin_id}find_command")
        ...
        if command_str in [
            commandprefix,
            f"{commandprefix}.",
            f"{commandprefix}.plugins",
            f"{commandprefix}.plugins.",
        ]:
            return None, "", False, "Proxy Help", self.find_command_only_prefix()

        # split the string into the command and the command_args
        found, new_package, new_plugin, tmessage, command_split, command_args = (
            self.find_command_split_command_string(command_str)
        )
        ...
        LogRecord(
            f"{found} {new_package} {new_plugin} {tmessage}",
            level="debug",
            sources=[self.plugin_id],
        )(actor=f"{self.plugin_id}:find_command")
        ...
        LogRecord(f"{temp_package=}", level="debug", sources=[self.plugin_id])(
            actor=f"{self.plugin_id}:find_command"
        )
        ...
        LogRecord(f"{command_list=}", level="debug", sources=[self.plugin_id])(
            actor=f"{self.plugin_id}:find_command"
        )
        LogRecord(f"{temp_command=}", level="debug", sources=[self.plugin_id])(
            actor=f"{self.plugin_id}:find_command"
        )

        new_command = self.api("plugins.core.fuzzy:get.best.match")(
            temp_command, tuple(command_list), scorer="token_set_ratio"
        )
    ...
    def find_command_split_command_string(
        ...
        LogRecord(
            f"looking for {command_str}, {command_str}, {command_args}",
            level="debug",
            sources=[self.plugin_id],
        )(actor=f"{self.plugin_id}:find_command")
        ...
        LogRecord(f"{command_split=}", level="debug", sources=[self.plugin_id])(
            actor=f"{self.plugin_id}:find_command"
        )
        ...
        LogRecord(f"2: {command_split=}", level="debug", sources=[self.plugin_id])(
            actor=f"{self.plugin_id}:find_command"
        )
        ...
        new_package, new_plugin = self.api("libs.plugins.loader:fuzzy.match.plugin.id")(
            plugin_id_temp_str, active_only=True
        )
    ...
    def run_internal_command_from_event(self):
        ...
        command_item, command_args, show_in_history, notes, message = self.find_command(
        ...
        if message:
            if isinstance(message, list):
                message = NetworkData(
                    message, owner_id="run_internal_command_from_event"
                )
        ...
        if event_record["showinhistory"] != show_in_history:
            ...
            event_record.addupdate(
                "Modify", "show_in_history set to {show_in_history}", savedata=False
            )

        event_record.addupdate(
            "Info",
            f"find_command returned {notes}, arg string: '{command_args}'",
            savedata=False,
        )

        if command_item:
            LogRecord(
                f"found command {command_item.plugin_id}.{command_item.name}",
                level="debug",
                sources=[self.plugin_id],
            )(actor=f"{self.plugin_id}:run_internal_command_from_event")
            ...
            if success:
                ...
                event_record.addupdate(
                    "Info", f"run_command returned error: {error}", savedata=False
                )
    ...
    def _eventcb_check_for_command(self) -> None:
        ...
        if self.current_input_event == event_record:
            LogRecord(
                f"current_input_event: {self.current_input_event} is being processed, skipping",
                level="debug",
                sources=[self.plugin_id],
            )()
    ...
    def _api_remove_command(self, plugin_id, command_name):
        ...
        if not self.api("libs.plugins.loader:is.plugin.id")(plugin_id):
            LogRecord(
                f"remove command: plugin {plugin_id} does not exist",
                level="warning",
                sources=[self.plugin_id, plugin_id],
            )(f"{self.plugin_id}:_api_remove_command")
            return False
        ...
        if data and command_name in data:
            ...
            LogRecord(
                f"removed command {plugin_id}.{command_name}",
                level="debug",
                sources=[self.plugin_id, plugin_id],
            )()
            return True
        ...
        LogRecord(
            f"remove command: command {plugin_id}.{command_name} does not exist",
            level="error",
            sources=[self.plugin_id, plugin_id],
        )()
        return False
        ...
        for i in command_list:
            if i != "default" and i.arg_parser.description:
                ...
                if not tlist[0]:
                    tlist.pop(0)
                message.append(
                    f"  {output_header_color}{i.name.replace('plugins.', ''):<10}@w : {tlist[0]}"
                )

        return message

    @AddAPI(
        "list.commands.formatted",
    ...
    def _api_list_commands_formatted(self, plugin_id):
        ...
        output_subheader_color = self.api("plugins.core.settings:get")(
            self.plugin_id, "output_subheader_color"
        )
        ...
        message.append(
            output_subheader_color + "-" * 5 + " " + "Base" + " " + "-" * 5 + "@w"
        )
    ...
    def _api_list_plugins_formatted(self, package=None):
        ...
        if package:
            plugin_id_list = [
                ...
                for item in self.api("libs.plugins.loader:get.plugins.in.package")(
                    package
                )
            ]
        else:
            plugin_id_list = [
                item.replace("plugins.", "")
                for item in self.api("libs.plugins.loader:get.loaded.plugins.list")()
            ]
    ...
    @AddArgument("plugin", help="the plugin to see help for", default="", nargs="?")
    @AddArgument(
        "command",
    ...
    def _command_list(self, _=None):
        ...
        if not self.api("libs.plugins.loader:is.plugin.id")(plugin_id):
            return True, self.api(f"{self.plugin_id}:list.plugins.formatted")(
                package=plugin_id
            )
        ...
        if plugin_commands := self.command_data[plugin_id]:
            if command and command in plugin_commands:
                help_message = (
                    plugin_commands[command].arg_parser.format_help().splitlines()
                )
                message.extend(help_message)
            else:
                message.extend(
                    self.api(f"{self.plugin_id}:list.commands.formatted")(plugin_id)
                )
        else:
    ...
    def _command_run_history(self):
        ...
        SendDataDirectlyToClient(
            NetworkData(
                [f"Commands: rerunning command {command}"], owner_id="run_history"
            )
    ...
    @AddCommand(shelp="list or run a command in history", show_in_history=False)
