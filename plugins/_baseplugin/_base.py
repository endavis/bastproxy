...
class Plugin:  # pylint: disable=too-many-instance-attributes
    ...
    def _process_plugin_hook(self, plugin_hook, **kwargs) -> dict:
        ...
        for key in sorted_keys:
            for func in functions[key]:
                LogRecord(
                    f"phook {plugin_hook} calling function {func.__name__}",
                    level="debug",
        ...
        if (
            ...
            for version in range(old_plugin_version + 1, new_plugin_version + 1):
                ...
                if version in self.version_functions:
                    ...
                    LogRecord(
                        f"_update_version: no function to upgrade to version {version}",
                        level="error",
                        sources=[self.plugin_id, "plugin_upgrade"],
                    )()
    ...
    def _phook_base_post_initialize_setup(self):
        ...
        if (
            self.api("plugins.core.proxy:is.mud.connected")
            and self.api("libs.api:is.character.active")()
        ):
        ...
        if not self.api.startup:
            ...
            self.api("plugins.core.events:raise.event")(
                "ev_plugin_loaded", event_args={"plugin_id": self.plugin_id}
            )
    ...
    def _eventcb_baseplugin_after_character_is_active(self):
        ...
        if self.api("plugins.core.events:is.registered.to.event")(
            "ev_libs.api_character_active",
            self._eventcb_baseplugin_after_character_is_active,
        ):
    ...
    def _phook_base_unitialize_hook(self):
        ...
        if self.is_reloading_f:
            for item in self.attributes_to_save_on_reload:
                self.api("libs.plugins.reloadutils:add.cache")(
                    self.plugin_id, item, self.__getattribute__(item)
                )

        self.api("plugins.core.events:raise.event")(f"ev_{self.plugin_id}_unloaded")
        self.api("plugins.core.events:raise.event")(
            "ev_plugin_unloaded", event_args={"plugin_id": self.plugin_id}
        )
    ...
    def _phook_base_initialize(self):
        ...
        self.api("plugins.core.events:register.to.event")(
            "ev_libs.net.mud_muddisconnect", self._eventcb_baseplugin_disconnect
        )

        self.api("plugins.core.events:add.event")(
            f"ev_{self.plugin_id}_savestate",
            self.plugin_id,
    ...
    def _api_save_state(self):
        ...
        self.api("plugins.core.events:raise.event")(
            "ev_plugin_save", event_args={"plugin_id": self.plugin_id}
        )
